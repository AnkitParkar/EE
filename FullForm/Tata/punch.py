...
def csv_access(type,model,color,bookPrice,showPrice):
    ...
    if str(sp)!=str(showPrice): Error_log("punch",model,type,"Showroom Price",showPrice,sp,driver.current_url,json_data['error_csv_path'])
    ...
    temp = cur.text
    sp = "".join([i for i in temp if i.isdigit()])
    if sp != str(showPrice): Error_log("punch", model, type, "Showroom Price", showPrice, sp, driver.current_url,json_data['error_csv_path'])
...
def select_variant():
    ...
    col.click()
    model=col.text.split('Punch ')[1].strip()
    time.sleep(4)
    ...
    if (random.randint(1, 2) == 1):
        driver.find_element(By.XPATH,"/html/body/app-root/div/app-variants/div[1]/div[1]/form/div[3]/div[1]/div/div[1]/div[2]/div/label").click()
        time.sleep(3)
        acc=car_df['Acc'].to_string(index=False).split('.')[0]
        showPrice=str(int(showPrice)+int(acc))
